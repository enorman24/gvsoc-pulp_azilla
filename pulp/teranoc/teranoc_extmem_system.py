...
class TeranocSystem(st.Component):
    ...
    def __init__(self, parent, name, parser, arch, binary=None):
        ...
        soc_ico.add_mapping('sharedmem', base=0xC1000000, size=0x01000000, latency=1)
        ...
        # Shared-memory router: dedicated so its bandwidth matches real AXI
        # traffic instead of ext_ico's UART-sized bandwidth=4.
        sharedmem_ico = router.Router(self, 'sharedmem_ico', bandwidth=axi_data_width)
        sharedmem_ico.add_mapping('proxy', base=0xC1000000, remove_offset=0xC1000000, size=0x01000000, latency=1)

        # Bridge to SST: forwards io requests across the process boundary
        sharedmem_proxy = router_proxy.Router_proxy(self, 'sharedmem_proxy')
        ...
        # Shared memory -> SST
        self.bind(soc_ico, 'sharedmem', sharedmem_ico, 'input')
        self.bind(sharedmem_ico, 'proxy', sharedmem_proxy, 'input')
