...
"""io_v2 port of the Spatz cluster peripheral registers.

Same regmap (``spatz_cluster_peripheral_reg.hjson``) and behaviour as the
v1 ``cluster_registers.py``; the ``input`` / ``input_<core>`` ports speak
io_v2. The HW barrier stalls a core by answering ``IO_REQ_GRANTED`` and
responding once all cores reached the barrier.
"""
...
class ClusterRegisters(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, boot_addr=0, nb_cores=1, binary=None):
        ...
        self.add_sources(['pulp/snitch/snitch_cluster/spatz/cluster_registers_v2.cpp'])
